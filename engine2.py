...
class ChessAi:
    ...
    def request_piece_move(self, x, y, flipped):
        ...
        for move in moves:
            ...
            if X == x and Y == y and not flipped:
                marker_location.append((int(columns.index(str(move)[2])), -int(str(move)[3]) + 8))
            if -X + 7 == x and Y == y and flipped:
                marker_location.append((-int(columns.index(str(move)[2])) + 7, int(str(move)[3]) - 1))
        return marker_location

    def coor_move(self, X, Y, x, y, flipped, promotion=""):
        columns = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']
        if not flipped:
            self.board.push(chess.Move.from_uci(str(columns[X] + str(Y) + str(columns[x] + str(y) + promotion))))
        if flipped:
    ...
    def make_move(self, player, flipped):
        print(player)
        move = MM.find_best_move(self.board, self.depth, player, chess.Board.push, lambda position: list(position.legal_moves), chess.Board.is_game_over, evalboard, bias)
        return move
