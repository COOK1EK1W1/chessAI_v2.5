...
class Screen:
    ...
    def add_button(self, x, y, width, height, name=None, func=None, colour=(0,0,0), toggle=None, toggle_colour=(255,255,255)):
        if name == None:
            self.buttons.append(Screen.Button(self, x, y, width, height, len(self.buttons), func, colour, toggle, toggle_colour))
        else:
            self.buttons.append(Screen.Button(self, x, y, width, height, name, func, colour, toggle, toggle_colour))
                    
    class Button:
        def __init__(self, screen, x, y, width, height, name, func=False, colour=(0,0,0), toggle=None, toggle_colour=(255,255,255)):
            self.screen = screen
            self.name = name
            self.x = x
            self.y = y
        ...
        def update(self, new_colour):
            ...
            pygame.draw.rect(screen.DISPLAYSURF, self.current_colour, (self.x , self.y , self.width, self.height))
